...
export default function DashboardIAPage() {
    ...
    useEffect(() => {
        carregarDadosIA();
        // Atualização em tempo real via SSE (EventSource)
        let es: EventSource | null = null;
        try {
            const streamUrl = buildApiUrl('/api/relatorios/ia/dashboard/stream');
            es = new EventSource(streamUrl);
            es.onopen = () => setSseActive(true);
            es.onmessage = (ev) => {
                try {
                    const payload = JSON.parse(ev.data);
                    const dadosProcessados = processarDadosReais(payload);
                    setDados(dadosProcessados);
                } catch {}
            };
            es.onerror = () => setSseActive(false);
        } catch {}
        // Fallback com polling leve
        const pollId = setInterval(() => carregarDadosIA(), 30000);
        return () => {
            if (es) es.close();
            setSseActive(false);
    ...
    const carregarDadosIA = async () => {
        ...
        try {
            ...
            const dadosIA = await fetchWithCache<any>('/api/relatorios/ia/dashboard', 'relatorios');
            if (dadosIA) {
    ...
    const processarDadosReais = (dadosIA: unknown): PrevisaoData => {
        const src = dadosIA as Record<string, unknown>;
        console.log('Dados recebidos da API:', dadosIA);
        
        // Converter os dados da API para o formato esperado
        const dadosProcessados: PrevisaoData = {
            previsao1h: Math.round(Number((src.previsao1h as number) || 0) * 100),
            picoMaximo: Math.round(Number((src.picoMaximo as number) || 0) * 100),
            confiancaMedia: Math.round(Number((src.confiancaMedia as number) || 0) * 100),
            tendencia: (src.tendencia as 'crescendo'|'diminuindo'|'estavel') || 'estavel',
            dadosGrafico: Array.isArray((src as any).dadosGrafico) ? ((src as any).dadosGrafico as Array<Record<string, unknown>>).map((item) => ({
                hora: String(item.hora || ''),
                ocupacao: Math.round(Number((item.ocupacao as number) || 0) * 100),
                previsao: Math.round(Number((item.previsao as number) || 0) * 100),
                confianca: Math.min(Math.round(Number((item.confianca as number) || 0)), 100)
            })) : []
        };
                ...
                {!loading && (dados.dadosGrafico.length > 0 || dados.previsao1h > 0) && (
                    <>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="neumorphic-container p-6">
                                <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2" style={{fontFamily: 'Montserrat, sans-serif'}}>
                                    <i className="ion-ios-shield text-amber-500"></i>
                                    ...
                                    {dados.dadosGrafico.slice(0, 6).map((item, index) => (
                                        <div key={index} className="flex items-center justify-between">
                                            <span className="text-sm text-gray-700 flex items-center gap-2">
                                                <i className={`ion-ios-checkmark-circle ${getConfiancaColor(item.confianca || 0)}`}></i>
                                                {item.hora}
                                            </span>
                                            <div className="flex items-center">
