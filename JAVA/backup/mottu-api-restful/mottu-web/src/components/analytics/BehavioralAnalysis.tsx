...
export default function BehavioralAnalysis({ periodo, filtro = 'todos' }: BehavioralAnalysisProps) {
  ...
  const fetchBehavioralData = async () => {
    try {
      ...
      const clientesData = await fetchWithCache<any>('/api/clientes/search', 'entidades');
      ...
      const patiosData = await fetchWithCache<any>('/api/patios/search', 'mapas');
      ...
      for (const cliente of clientesBackend.slice(0, 10)) { // Limitar a 10 clientes para performance
        try {
          ...
          const veiculosData = await fetchWithCache<any>(`/api/veiculos/search?clienteId=${cliente.idCliente}`, 'entidades');
