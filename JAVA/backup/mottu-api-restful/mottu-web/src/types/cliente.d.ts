...
// --- DTOs para CNH ---
export interface CnhRequestDto {
    dataEmissao: string; // "YYYY-MM-DD"
    dataValidade: string; // "YYYY-MM-DD"
    numeroRegistro: string;
    categoria: 'A' | 'B' | 'C' | 'D' | 'E' | 'AB' | 'AC' | 'AD' | 'AE';
    clienteId?: number; // Opcional - será definido pelo backend na criação
}

export interface CnhResponseDto {
    idCnh: number;
    dataEmissao: string;
    dataValidade: string;
    numeroRegistro: string;
