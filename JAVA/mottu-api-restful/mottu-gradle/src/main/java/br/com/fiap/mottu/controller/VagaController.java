...
public class VagaController {
    ...
    public ResponseEntity<Map<String, Object>> buscarPlaca(@PathVariable String placa) {
        return service.buscarBoxPorPlaca(placa)
                .<ResponseEntity<Map<String, Object>>>map(b -> ResponseEntity.ok(new LinkedHashMap<>() {{
                    put("found", true);
                    put("placa", placa.toUpperCase());
                    put("boxId", b.idBox());
                    put("boxNome", b.nomeBox());
                    put("status", b.status());
                    put("patioId", b.patioId());
                }}))
    ...
    @PostMapping("/associar-por-prefixo")
    @Operation(summary = "Associar boxes por prefixo ao pátio", description = "Atualiza TB_BOX.TB_PATIO_ID_PATIO para boxes cujo nome começa com os prefixos informados (case-insensitive)")
    public ResponseEntity<Map<String, Object>> associarPorPrefixo(@RequestBody Map<String, Object> body) {
        try {
            Long patioId = ((Number) body.get("patioId")).longValue();
            @SuppressWarnings("unchecked")
            List<String> prefixos = (List<String>) body.get("prefixos");
            int count = service.associarBoxesPorPrefixos(patioId, prefixos);
            return ResponseEntity.ok(Map.of("ok", true, "atualizados", count));
        } catch (Exception e) {
