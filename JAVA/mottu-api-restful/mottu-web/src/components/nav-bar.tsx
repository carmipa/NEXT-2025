        ...
        subGroups: [
            {
                ...
                icon: "ion-ios-podium",
                items: [
                    { href: "/relatorios", label: "Tela Principal", icon: "ion-ios-home" },
...
export default function NavBar({ active }: { active?: string }) {
    ...
    const isItemActive = (item: typeof navItems[0]) => {
        // Itens folha: ativo apenas em match exato
        if (item.href) {
            return pathname === item.href;
        }
        if (active && item.label.toLowerCase().includes(active.toLowerCase())) return true;
        if (item.basePath) {
            // Não destacar 'Ajuda' quando estamos na rota específica de Feedback
            if (item.basePath === '/ajuda' && pathname?.startsWith('/ajuda/feedback')) {
                return false;
            }
            // Destaque apenas quando na raiz do menu ou em suas subseções (exceto casos acima)
