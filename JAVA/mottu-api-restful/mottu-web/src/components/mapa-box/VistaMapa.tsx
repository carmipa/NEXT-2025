        ...
        if (mapRef.current && coordenadas) {
            (async () => {
                ...
                if (!mapInstanceRef.current) {
                    ...
                    (mapInstanceRef.current as any).setView([coordenadas.lat, coordenadas.lng], 15);
                }

                (mapInstanceRef.current as any).eachLayer((layer: unknown) => {
                    if ((layer as any) instanceof L.Marker) {
                        (layer as any).remove();
                    }
                });

                if (patioAtual) {
                    const marker = L.marker([coordenadas.lat, coordenadas.lng]).addTo((mapInstanceRef.current as any));
