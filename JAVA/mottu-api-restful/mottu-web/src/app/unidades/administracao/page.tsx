...
// ---------- SERVICES ----------
import api from "@/utils/api"; // seu axios base, já existente no projeto (NEXT_PUBLIC_API_BASE_URL etc.)
...
function PillTab({
    ...
    return (
        ...
        ${active ? "bg-white text-[var(--color-mottu-dark)] shadow" : "/30 text-white hover:/40"}`}
            aria-pressed={active}
            role="tab"
        >
            {icon} {label}
                        ...
                        {pageInfo && pageInfo.totalPages > 1 && (
                            <div className="flex items-center gap-2 mt-4">
                                <button
                                    ...
                                    className="px-3 py-1 /30 text-white rounded disabled:opacity-50 flex items-center gap-1"
                                >
        ...
        <>
            <main className="min-h-screen  text-white">
                ...
                <footer className="container mx-auto px-4 pb-10 text-center text-white/50 text-xs">
                    ...
                    {" "}<Link href="/box/listar" className="underline">/box/*</Link>.
                </footer>
            </main>
            <style jsx global>{`
