...
import { getPatioByBoxName, getMapaUrl as getMapaUrlFromConfig, PATIOS_CONFIG } from "@/lib/patioConfig";
...
function MapaVagasContent() {
    ...
    const patioIdParam = params.get("patioId");
    ...
    useEffect(() => {
        ...
        const fetchMapa = async () => {
            ...
            if (patioIdParam) {
                patioId = Number(patioIdParam);
                url = `/api/vagas/mapa?patioId=${patioId}`;
                console.log('🧭 Definido por query patioId:', patioId);
            } else if (placaParam) {
                ...
                try {
                    ...
                    if (buscaRes.ok) {
                        ...
                        if (buscaData.found && buscaData.boxId) {
                            // 1) Preferir o patioId retornado pela API, se existir
                            if (buscaData.patioId) {
                                patioId = Number(buscaData.patioId);
                                url = `/api/vagas/mapa?patioId=${patioId}`;
                                console.log(`✅ Pátio determinado pelo backend: ID ${patioId}`);
                            } else {
                                // 2) Fallback: tentar deduzir pelo nome do box
                                ...
                                if (patioConfig) {
                                    ...
                                    console.warn('❌ Pátio não encontrado para box (fallback):', boxNome);
                                }
                            }
                        }
                    }
            ...
            // Fallback: se API não informar rows/cols, calcular automaticamente pela quantidade de vagas
            if ((!data.rows || !data.cols) && Array.isArray(data.boxes)) {
                const total = Math.max(1, data.boxes.length);
                const colsAuto = Math.ceil(Math.sqrt(total));
                const rowsAuto = Math.ceil(total / colsAuto);
            ...
            if (alive) {
                ...
                // Fallback adicional: se vier "box" na query e ainda não há nome, usar prefixo do box
                if (!patioName && boxNomeParam) {
                    const prefix = boxNomeParam.replace(/\d+.*/, '').toUpperCase();
                    setPatioName(prefix ? `Pátio ${prefix}` : 'Pátio Desconhecido');
                }
        ...
        <>
            <main className="min-h-screen text-white p-4 sm:p-6">
                <div className="mx-auto max-w-5xl">
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="py-2 sm:py-3 px-2 sm:px-4 font-mono text-xs sm:text-sm">
                                                        {ocupado ? (isHighlighted ? (placaParam || b.veiculo?.placa || 'N/A') : (b.veiculo?.placa || 'N/A')) : '-'}
                                                    </td>
					...
					{/* Substituído: link dedicado para o Mapa de Vagas (mapa-box) */}
					<div className="mt-4 sm:mt-8 rounded-2xl border border-zinc-700 bg-zinc-900 p-4 text-center">
						<Link href="/mapa-box" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-700 hover:bg-blue-600 text-white text-sm sm:text-base">
							<i className="ion-ios-map"></i>
							<span style={{fontFamily: 'Montserrat, sans-serif'}}>Abrir Mapa de Vagas</span>
						</Link>
					</div>
                </div>
