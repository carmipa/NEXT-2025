...
export default function ComportamentalPage() {
    ...
    useEffect(() => {
        ...
        // SSE: reutiliza stream de movimentação para atualizar comportamento em tempo real
            ...
            es.onmessage = (ev) => {
                try {
                    ...
                    if (Array.isArray(payload.detalhes)) {
                        processarMovsParaClientes(payload.detalhes);
                    }
                } catch {}
            };
            es.onerror = () => setSseActive(false);
        } catch {}
        const id = setInterval(carregarDadosComportamentais, 30000);
    ...
    const carregarDadosComportamentais = async () => {
        ...
        try {
            ...
            if (!responseMovimentacoes.ok) {
                ...
                // mantém últimos dados exibidos; apenas retorna
                return;
            }
            
            const movimentacoes = await responseMovimentacoes.json();
            if (Array.isArray(movimentacoes) && movimentacoes.length > 0) {
                processarMovsParaClientes(movimentacoes);
            }
            
        } catch (error) {
            console.error('❌ Erro ao carregar dados comportamentais:', error);
            // Em erro, mantém os dados atuais para evitar "piscar"/desaparecer
        } finally {
            setLoading(false);
        }
    };

    const processarMovsParaClientes = (movimentacoes: any[]) => {
        if (!Array.isArray(movimentacoes) || movimentacoes.length === 0) {
            // Não sobrescreve com vazio vindo do stream
            return;
        }
        ...
        const clientesProcessados: Cliente[] = Array.from(clientesData.entries()).map(([placa, dados]) => {
            ...
            const visitaMaisComum = Array.from(dados.pátios.entries()).sort((a, b) => b[1] - a[1])[0];
                            ...
                            <div key={cliente.id} className="neumorphic-container p-4 hover:scale-105 transition-all duration-300">
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between">
                                        <span className="text-gray-600 flex items-center gap-1"><i className="ion-ios-analytics"></i>Visitas:</span>
                                        <span className="font-semibold text-blue-600">{cliente.visitas}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-600 flex items-center gap-1"><i className="ion-ios-time"></i>Tempo Médio:</span>
                                        <span className="font-semibold text-green-600">{cliente.tempoMedio}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-600 flex items-center gap-1"><i className="ion-ios-home"></i>Pátio Preferido:</span>
                                        <span className="font-semibold text-purple-600">{cliente.patioPreferido} ({cliente.percentualPreferido}%)</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-600 flex items-center gap-1"><i className="ion-ios-calendar"></i>Última visita:</span>
                                        <span className="font-semibold text-gray-600">{cliente.ultimaVisita}</span>
                                    </div>
                                    {cliente.proximaVisita && (
                                        <div className="flex justify-between">
                                            <span className="text-gray-600 flex items-center gap-1"><i className="ion-ios-alarm"></i>Próxima visita:</span>
