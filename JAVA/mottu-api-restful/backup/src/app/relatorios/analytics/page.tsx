...
import { ResponsiveContainer, LineChart, Line, CartesianGrid, XAxis, YAxis, Tooltip, Legend, BarChart, Bar, PieChart, Pie, Cell, Area, ComposedChart } from 'recharts';

type Serie = { t: string; eventos: number; veiculos: number; conversao: number; previsto?: number; real?: number; erroPct?: number };
...
export default function AnalyticsRelatorioPage() {
  ...
  // Mapa de cores do erro (%) → gradiente verde → amarelo → laranja → vermelho → lilás
  const getErrorColor = (pct: number) => {
    if (pct < 5) return '#10b981';        // verde
    if (pct < 10) return '#84cc16';       // verde-lima
    if (pct < 20) return '#f59e0b';       // amarelo
    if (pct < 30) return '#f97316';       // laranja
    if (pct < 50) return '#ef4444';       // vermelho
    if (pct < 75) return '#a855f7';       // lilás
    return '#7c3aed';                     // roxo forte
  };
  ...
  const carregar = async () => {
    ...
    setSerie(prev => {
      ...
      const erroPct = ponto.real && previstoAtual ? Math.abs(ponto.real - previstoAtual) / Math.max(1, ponto.real) * 100 : 0;
      const next = [...prev, { ...ponto, previsto: +previstoAtual.toFixed(2), erroPct: +erroPct.toFixed(1) }];
    ...
    try {
      ...
      es.onmessage = (ev) => {
        try {
          ...
          if (Array.isArray(payload?.topVeiculos)) setTopVeiculos(payload.topVeiculos);
          if (Array.isArray(payload?.topBoxes)) setTopBoxes(payload.topBoxes);
          if (Array.isArray(payload?.topPatios)) setTopPatios(payload.topPatios);
          // também empurra a série usando os KPIs
          if (payload?.kpi) {
            const ponto: Serie = {
              t: new Date().toLocaleTimeString('pt-BR', { hour12: false, hour: '2-digit', minute: '2-digit' }),
              eventos: payload.kpi.totalEventos || 0.1,
              veiculos: payload.kpi.veiculosUnicos || 0.1,
              conversao: payload.kpi.conversao || 0.1,
              real: payload.kpi.totalEventos || 0.1
            };
            setSerie(prev => {
              const alpha = 0.35;
              const prevPrevisto = prev.length ? prev[prev.length - 1].previsto ?? prev[prev.length - 1].real ?? ponto.real : ponto.real;
    ...
    <div className="min-h-screen bg-black relative">
      ...
      <div className="relative z-10 p-6">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl p-6 border border-white/20">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <label className="flex items-center gap-2 text-white/80 text-sm"><input type="checkbox" checked={useLog} onChange={(e)=>setUseLog(e.target.checked)} /> Escala log</label>
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={serie}>
                  ...
                  <XAxis dataKey="t" />
                  <YAxis scale={useLog ? 'log' : undefined} domain={useLog ? [0.1,'auto'] : ['auto','auto']} />
                  <Tooltip />
                  <Legend />
          ...
          {/* Previsões vs Realidade (composed: área previsto + linha real + erro %) */}
          <div className="neumorphic-container">
            <div className="flex items-center justify-between mb-3">
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={serie}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="t" />
                  <YAxis yAxisId="esq" scale={useLog ? 'log' : undefined} domain={useLog ? [0.1,'auto'] : ['auto','auto']} />
                  <YAxis yAxisId="dir" orientation="right" domain={[0, 100]} />
                  <Tooltip formatter={(v:any, n:any)=> n==='Erro %' ? [`${v}%`, n] : [v, n]} />
                  <Legend />
                  <Area yAxisId="esq" type="monotone" dataKey="previsto" name="Previsto" stroke="#60a5fa" fill="#60a5fa" fillOpacity={0.35} />
                  <Line yAxisId="esq" type="monotone" dataKey="real" name="Real" stroke="#f87171" strokeWidth={2} dot={{ r: 2, fill: '#f87171' }} />
                  <Bar yAxisId="dir" dataKey="erroPct" name="Erro %" barSize={10}>
                    {serie.map((s, i) => (
                      <Cell key={`err-${i}`} fill={getErrorColor(s.erroPct || 0)} />
                    ))}
                  </Bar>
                </ComposedChart>
              </ResponsiveContainer>
              ...
              <div className="h-72">
                {topVeiculos.length === 0 ? (
                  <div className="w-full h-full flex items-center justify-center text-gray-400 text-sm">
                    Sem dados para exibir. Gere movimentações para popular este gráfico.
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="neumorphic-container">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="h-72">
                {topPatios.length === 0 ? (
                  <div className="w-full h-full flex items-center justify-center text-gray-400 text-sm">
                    Sem dados para exibir. Cadastre pátios/boxes e realize entradas/saídas.
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="neumorphic-container">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="h-72">
                {topBoxes.length === 0 ? (
                  <div className="w-full h-full flex items-center justify-center text-gray-400 text-sm">
                    Sem dados para exibir. Movimente veículos entre boxes para ver o ranking.
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
