      ...
      es.onmessage = (ev) => {
        try {
          ...
          if (Array.isArray(payload.porPatio)) {
            ...
            const totalLivres = payload.porPatio.reduce((a: number, p: any) => a + (p.boxesLivres || 0), 0);
            const totalOcupados = payload.porPatio.reduce((a: number, p: any) => a + (p.boxesOcupados || 0), 0);
            const totalManut = payload.porPatio.reduce((a: number, p: any) => a + (p.boxesManutencao || 0), 0);
            const ponto: SerieManutencao = {
              t: new Date().toLocaleTimeString('pt-BR', { hour12: false, hour: '2-digit', minute: '2-digit' }),
              livres: totalLivres || 0.1,
              ocupados: totalOcupados || 0.1,
