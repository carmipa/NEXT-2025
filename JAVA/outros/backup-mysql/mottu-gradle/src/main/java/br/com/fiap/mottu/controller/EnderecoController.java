...
public class EnderecoController {
    ...
    @Operation( // Documentação da operação via Swagger
            summary = "Listar todos os endereços (sem paginação)",
            description = "Retorna uma lista de todos os endereços cadastrados para uso em formulários e seleções.",
            responses = {
    ...
    @GetMapping("/all") // Mapeia requisições GET para /api/enderecos/all
    public ResponseEntity<List<EnderecoResponseDto>> listarTodosEnderecos() {
        log.info("Buscando todos os endereços (sem paginação)");
        List<Endereco> enderecos = enderecoService.listarTodosEnderecos(); // Chama o serviço
        ...
        log.info("Retornando {} endereços.", enderecosDto.size());
        return ResponseEntity.ok(enderecosDto); // Retorna a lista de DTOs
    }
    ...
    public ResponseEntity<Page<EnderecoResponseDto>> listarTodosEnderecosPaginado(
