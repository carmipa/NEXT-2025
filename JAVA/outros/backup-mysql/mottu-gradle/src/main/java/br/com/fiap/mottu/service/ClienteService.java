...
import br.com.fiap.mottu.model.Cnh; // Entidade CNH
...
import br.com.fiap.mottu.repository.CnhRepository; // Repositório para CNH
...
import br.com.fiap.mottu.mapper.CnhMapper; // Mapper para CNH
...
public class ClienteService {
    ...
    private static final Logger log = LoggerFactory.getLogger(ClienteService.class); // Logger para esta classe
    ...
    private final CnhRepository cnhRepository; // Repositório para CNH
    ...
    private final CnhMapper cnhMapper; // Mapper para CNH
    ...
    public Page<Cliente> listarTodosClientes(Pageable pageable) {
        log.info("Iniciando listagem de clientes", 
            LoggingConfig.LogContext.create()
                .addOperation("listarTodosClientes")
                .addEntityType("Cliente")
        ...
        try {
            ...
            log.info("Listagem de clientes concluída com sucesso", 
                LoggingConfig.LogContext.create()
                    .addOperation("listarTodosClientes")
                    .addEntityType("Cliente")
    ...
    public Cliente buscarClientePorId(Long id) {
        log.info("Buscando cliente por ID", 
            LoggingConfig.LogContext.create()
                .addOperation("buscarClientePorId")
                .addEntityType("Cliente")
                .addEntityId(id)
                .build());
    ...
    public Mono<Cliente> criarCliente(ClienteRequestDto dto) { //
        log.info("Iniciando criação de cliente", 
            LoggingConfig.LogContext.create()
                .addOperation("criarCliente")
                .addEntityType("Cliente")
                .add("nome", dto.getNome())
                .add("cpf", dto.getCpf())
                .build());
        ...
        return Mono.zip(enderecoMono, contatoMono) //
                .flatMap(tuple -> {
                    ...
                    // Cria a CNH associada ao cliente
                    CnhRequestDto cnhDto = dto.getCnhRequestDto();
                    if (cnhDto != null) {
                        // Verificar se o número de registro da CNH já existe
                        if (cnhRepository.existsByNumeroRegistro(cnhDto.getNumeroRegistro())) {
                            throw new DuplicatedResourceException("Número de registro da CNH já existe: " + cnhDto.getNumeroRegistro());
                        }
                        
                        // Criar CNH com o ID do cliente salvo
                        CnhRequestDto cnhComClienteId = new CnhRequestDto(
                            cnhDto.getDataEmissao(),
                            cnhDto.getDataValidade(),
                            cnhDto.getNumeroRegistro(),
                            cnhDto.getCategoria(),
                            clienteSalvo.getIdCliente() // Usar o ID do cliente salvo
                        );
                        
                        Cnh cnh = cnhMapper.toEntity(cnhComClienteId, clienteSalvo);
                        Cnh cnhSalva = cnhRepository.save(cnh);
                        
                        // Associar a CNH ao cliente
                        clienteSalvo.setCnh(cnhSalva);
                    }
                    
                    return Mono.just(clienteSalvo); // Retorna o cliente com CNH
                })
    ...
    public void deletarCliente(Long id) {
        Cliente cliente = clienteRepository.findById(id)
            .orElseThrow(() -> new ResourceNotFoundException("Cliente", id)); // Busca o cliente ou lança exceção
        
        // Deleta o cliente primeiro (isso remove as associações)
        clienteRepository.deleteById(id);
        
        // Verifica se o contato é exclusivo deste cliente antes de deletar
        Long contatoId = cliente.getContato().getIdContato();
        if (contatoId != null && !clienteRepository.existsByContatoIdContato(contatoId)) {
            contatoRepository.deleteById(contatoId);
        }
        
        // Verifica se o endereço é exclusivo deste cliente antes de deletar
        Long enderecoId = cliente.getEndereco().getIdEndereco();
        if (enderecoId != null && !clienteRepository.existsByEnderecoIdEndereco(enderecoId)) {
