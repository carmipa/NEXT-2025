            ...
            if (response.ok) {
                ...
                if (Array.isArray(boxesData) && boxesData.length > 0) {
                    return boxesData.map((box: any, index: number) => ({
                        ...
                        numero: box.numeroBox || index + 1,
                        status: box.status || (box.veiculo ? 'ocupado' : 'livre'),
                        veiculo: box.veiculo ? box.veiculo.placa : undefined,
                        placa: box.veiculo ? box.veiculo.placa : undefined
                    }));
                }
            } else {
                                    ...
                                    <div className="h-80">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <BarChart data={patiosFiltrados}>
                                                ...
                                                <YAxis />
                                                <Tooltip />
                                                <Bar dataKey="ocupacaoAtual" fill="#ef4444" name="Ocupação Atual" />
                                            </BarChart>
                                        </ResponsiveContainer>
                                    </div>
                                </div>
                                ...
                                <div className="neumorphic-container">
                                    ...
                                    <div className="h-80">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <PieChart>
                                                <Pie
                                                    data={patiosFiltrados.map(patio => ({
                                                        name: patio.nome,
                                                        value: patio.ocupacaoAtual,
                                                        color: getOcupacaoColor(patio.ocupacaoAtual)
                                                    }))}
                                                    cx="50%"
                                                    cy="50%"
                                                    labelLine={false}
                                                    ...
                                                    dataKey="value"
                                                >
                                                    {patiosFiltrados.map((entry, index) => (
                                                        <Cell key={`cell-${index}`} fill={getOcupacaoColor(entry.ocupacaoAtual)} />
                                                    ))}
                                                </Pie>
                                                <Tooltip />
                                                <Legend 
                                    ...
                                    <div className="h-80">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <LineChart data={patiosFiltrados}>
                                                ...
                                                <Tooltip />
                                                <Legend />
                                                <Line type="monotone" dataKey="ocupacaoAtual" stroke="#ef4444" strokeWidth={2} name="Ocupação Atual" />
                                                <Line type="monotone" dataKey="media" stroke="#3b82f6" strokeWidth={2} name="Média" />
                                                <Line type="monotone" dataKey="maxima" stroke="#f59e0b" strokeWidth={2} name="Máxima" />
                                            </LineChart>
                                        </ResponsiveContainer>
                                    </div>
                                </div>
