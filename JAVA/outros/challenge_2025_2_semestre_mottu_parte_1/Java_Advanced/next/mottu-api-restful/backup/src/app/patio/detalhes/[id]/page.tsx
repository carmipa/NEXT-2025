        ...
        <>
            ...
            <main className="flex justify-center items-center min-h-screen p-4">
                <div className="text-center bg-red-900/50 p-8 rounded-lg">
                    ...
                    <Link href="/gerenciamento-patio" className="mt-6 inline-flex items-center gap-2 px-6 py-2 bg-slate-600 text-white rounded-md"><ArrowLeft size={18}/> Voltar</Link>
                </div>
            </main>
        </>
    );
                ...
                <div className="container max-w-4xl mx-auto bg-[var(--color-mottu-default)] p-6 md:p-8 rounded-lg shadow-xl">
                    <div className="flex flex-col sm:flex-row justify-between items-start mb-6">
                        ...
                        <div className="flex gap-2 mt-4 sm:mt-0">
                            <Link href="/gerenciamento-patio" className="flex items-center gap-2 px-4 py-2 font-medium text-[var(--color-mottu-dark)] bg-white rounded-md shadow hover:bg-gray-100">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-black/20 p-4 rounded-lg">
                            <h2 className="text-xl font-semibold mb-3 text-slate-100 flex items-center"><Building className="mr-2"/>Dados do Pátio</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2 text-sm">
                                <p><strong>Nome:</strong> {patio.nomePatio}</p>
                                <p><strong>Status:</strong> {patio.status === 'A' ? 'Ativo' : 'Inativo'}</p>
                                <p><strong>Data Cadastro:</strong> {new Date(patio.dataCadastro).toLocaleDateString()}</p>
                        ...
                        <div className="bg-black/20 p-4 rounded-lg">
                            <h2 className="text-xl font-semibold mb-3 text-slate-100 flex items-center"><Phone className="mr-2"/>Contato Principal</h2>
                            {patio.contato ? (
                                <div className="space-y-2 text-sm">
                                    <p><strong>Email:</strong> <span className="text-sky-300">{patio.contato.email}</span></p>
                                    <p><strong>Celular:</strong> {patio.contato.celular}</p>
                                    <p><strong>Telefone:</strong> ({patio.contato.ddd}) {patio.contato.telefone1}</p>
                                </div>
                            ) : (
                                <p className="text-slate-400 text-sm">Nenhum contato principal associado a este pátio.</p>
                            )}
                        </div>

                        {/* CORREÇÃO: Exibição detalhada do Endereço Associado */}
                        <div className="bg-black/20 p-4 rounded-lg">
                            <h2 className="text-xl font-semibold mb-3 text-slate-100 flex items-center"><Home className="mr-2"/>Endereço Principal</h2>
                            {patio.endereco ? (
                                <div className="space-y-2 text-sm">
                                    <p><strong>Logradouro:</strong> {patio.endereco.logradouro}, {patio.endereco.numero}</p>
                                    <p><strong>Bairro:</strong> {patio.endereco.bairro}</p>
                                    <p><strong>Cidade/UF:</strong> {patio.endereco.cidade} / {patio.endereco.estado}</p>
                                    <p><strong>CEP:</strong> {patio.endereco.cep}</p>
                                </div>
                            ) : (
                                <p className="text-slate-400 text-sm">Nenhum endereço principal associado a este pátio.</p>
                            )}
                        </div>
                    </div>
                </div>
