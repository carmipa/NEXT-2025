...
const LOT_OUTLINE: [number, number][] = [ [0, 0], [60, 0], [60, 16], [50, 16], [50, 22], [64, 22], [64, 68], [6, 68], [6, 60], [0, 60], [0, 0],];
const ROOFS: { id: string; poly: [number, number][] }[] = [ { id: "telhado-principal", poly: LOT_OUTLINE } ];
const STREETS = [{ name: "R. Antônio Pegoraro", from: [-10, -6], to: [72, -6] },{ name: "Viela Espingarda", from: [70, 20], to: [70, 74] },{ name: "R. Maria Antonieta", from: [-8, 74], to: [66, 74] },];
...
export default function PatioMottuGuarulhos({ highlightBoxId }: { highlightBoxId?: string | null }) {
    ...
    const getBoxPositionFromName = (boxName: string) => {
        const match = boxName.match(/^(B|Li|GRU)(\d+)$/);
        ...
        if (prefixo === 'B' || prefixo === 'GRU') {
            // Boxes principais na área superior do pátio (dentro da área 0-60 x 0-16)
            baseX = 2; baseY = 2; cols = 12;
        } else if (prefixo === 'Li') {
            // Boxes de linha na área lateral (dentro da área 6-64 x 22-68)
            baseX = 8; baseY = 24; cols = 10;
        } else {
            // Posição padrão
            baseX = 2; baseY = 2; cols = 8;
        }
    ...
    useEffect(() => {
        const loadBoxData = async () => {
            ...
            setError(null);
            try {
                // Busca dados específicos do pátio Guarulhos (ID: 12)
                const response = await fetch('/api/vagas/mapa?patioId=12');
    ...
    const handleMouseEnterBox = (e: React.MouseEvent, box: MappedBox) => {
        if (box.status === 'Ocupado' && box.veiculo) {
            setTooltip({
                ...
                content: ( <div className="text-xs"> <div className="font-bold mb-1">Placa: {box.veiculo.placa}</div> <div><strong>Modelo:</strong> {box.veiculo.modelo}</div> <div><strong>Fabricante:</strong> {box.veiculo.fabricante}</div> <div><strong>Tag BLE:</strong> {box.veiculo.tagBleId || 'N/A'}</div> </div> )
            });
        }
    };
