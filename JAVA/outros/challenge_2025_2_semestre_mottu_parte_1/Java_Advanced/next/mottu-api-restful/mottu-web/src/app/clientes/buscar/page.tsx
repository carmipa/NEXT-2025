                ...
                <div className="container mx-auto">
                    <div className="neumorphic-container p-6 md:p-8">
                        <h1 className="flex items-center justify-center gap-2 text-2xl md:text-3xl font-bold mb-6 text-center text-slate-800" style={{fontFamily: 'Montserrat, sans-serif'}}>
                            <Search size={32} className="text-[var(--color-mottu-dark)]" /> Buscar Clientes
                        </h1>
                        ...
                        <fieldset className="neumorphic-fieldset mb-8">
                            ...
                            <form onSubmit={handleSearch} className="space-y-4 mt-6">
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 items-end">
                                    {/* Linha 1 */}
                                    <input type="text" name="nome" value={filter.nome || ''} onChange={handleFilterChange} className="neumorphic-input" placeholder="Nome" style={{fontFamily: 'Montserrat, sans-serif'}} />
                                    <input type="text" name="sobrenome" value={filter.sobrenome || ''} onChange={handleFilterChange} className="neumorphic-input" placeholder="Sobrenome" style={{fontFamily: 'Montserrat, sans-serif'}} />
                                    <IMaskInput mask="000.000.000-00" name="cpf" value={filter.cpf || ''} onAccept={(value) => handleMaskedFilterChange('cpf', value)} className="neumorphic-input" placeholder="CPF" style={{fontFamily: 'Montserrat, sans-serif'}} />
                                    <input type="email" name="contatoEmail" value={filter.contatoEmail || ''} onChange={handleFilterChange} className="neumorphic-input" placeholder="Email do contato" style={{fontFamily: 'Montserrat, sans-serif'}} />

                                    {/* Linha 2 */}
                                    <input type="text" name="enderecoCidade" value={filter.enderecoCidade || ''} onChange={handleFilterChange} className="neumorphic-input" placeholder="Cidade" style={{fontFamily: 'Montserrat, sans-serif'}} />
                                    <input type="text" name="enderecoEstado" value={filter.enderecoEstado || ''} onChange={handleFilterChange} className="neumorphic-input" placeholder="Estado (UF)" maxLength={2} style={{fontFamily: 'Montserrat, sans-serif'}}/>
                                    <input type="text" name="veiculoPlaca" value={filter.veiculoPlaca || ''} onChange={handleFilterChange} className="neumorphic-input" placeholder="Placa do Veículo" style={{fontFamily: 'Montserrat, sans-serif'}} />
                                    <input type="text" name="veiculoModelo" value={filter.veiculoModelo || ''} onChange={handleFilterChange} className="neumorphic-input" placeholder="Modelo do Veículo" style={{fontFamily: 'Montserrat, sans-serif'}} />

                                    <div className="lg:col-span-4 flex flex-col sm:flex-row justify-center items-center gap-3 pt-2">
                                        <button type="submit" className="neumorphic-button-green">
                                            <Search size={16} className="mr-2 inline" /> Buscar Clientes
                                        </button>
                                ...
                                {viewType === 'cards' ? (
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                                        {clientes.map((cliente) => (
                                            <div key={cliente.idCliente} className="neumorphic-card-gradient p-6 flex flex-col justify-between transition-all duration-300 hover:shadow-2xl hover:scale-105 transform hover:-translate-y-2 cursor-pointer">
                                                <div>
                                                ...
                                                <div className="flex justify-end items-center gap-2 pt-4 mt-auto">
                                                    <Link href={`/clientes/detalhes/${cliente.idCliente}`} className="p-2 rounded-full text-blue-600 hover:bg-blue-100 hover:scale-110 transition-all duration-300 hover:-translate-y-1" title="Ver Detalhes"><i className="ion-ios-eye text-xl"></i></Link>
                                                    <Link href={`/clientes/alterar/${cliente.idCliente}`} className="p-2 rounded-full text-yellow-500 hover:bg-yellow-100 hover:scale-110 transition-all duration-300 hover:-translate-y-1" title="Editar Cliente"><i className="ion-ios-create text-xl"></i></Link>
                                                    <Link href={`/clientes/deletar/${cliente.idCliente}`} className="p-2 rounded-full text-red-500 hover:bg-red-100 hover:scale-110 transition-all duration-300 hover:-translate-y-1" title="Excluir Cliente"><i className="ion-ios-trash text-xl"></i></Link>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                                        ...
                                                        <tr key={cliente.idCliente} className="hover:bg-slate-50 transition-colors">
                                                            ...
                                                            <td className="px-4 py-3">
                                                                <div className="flex justify-center items-center gap-2">
                                                                    <Link href={`/clientes/detalhes/${cliente.idCliente}`} className="p-1 rounded text-blue-600 hover:bg-blue-100 hover:scale-110 transition-all duration-300 hover:-translate-y-1" title="Ver Detalhes"><i className="ion-ios-eye"></i></Link>
                                                                    <Link href={`/clientes/alterar/${cliente.idCliente}`} className="p-1 rounded text-yellow-500 hover:bg-yellow-100 hover:scale-110 transition-all duration-300 hover:-translate-y-1" title="Editar Cliente"><i className="ion-ios-create"></i></Link>
                                                                    <Link href={`/clientes/deletar/${cliente.idCliente}`} className="p-1 rounded text-red-500 hover:bg-red-100 hover:scale-110 transition-all duration-300 hover:-translate-y-1" title="Excluir Cliente"><i className="ion-ios-trash"></i></Link>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                    ))}
