...
export default function PatioMottuLimao({ highlightBoxId }: { highlightBoxId?: string | null }) {
    ...
    const getBoxPositionFromName = (boxName: string) => {
        // Lógica adaptada para posicionar os boxes BLimao001-BLimao100 DENTRO do telhado
        ...
        if (prefixo === 'BLimao' || prefixo === 'B' || prefixo === 'LIMAO') { 
            ...
            // Layout: 20 colunas por linha para acomodar 100 boxes
            baseX = 4; baseY = 4; cols = 20; // 20 colunas = 5 linhas para 100 boxes
            w = 1.8; h = 3; // Boxes menores para acomodar 100 boxes
        }
        else if (prefixo === 'Li') { 
    ...
    useEffect(() => {
        const loadBoxData = async () => {
            ...
            setError(null);
            try {
                // Primeiro, busca o pátio Limão pelo nome para obter o ID correto
                ...
                if (!patioLimao) {
                    console.log('⚠️ Pátio Limão não encontrado. Usando pátio Guarulhos como fallback.');
                    // Se não encontrar Limão, usa Guarulhos como fallback
                ...
                console.log('🏢 Pátio Limão encontrado:', patioLimao.nomePatio, 'ID:', patioLimao.idPatio);
        ...
        const loadBoxesForPatio = async (patioId: number) => {
            try {
                // Limão tem ID 3 no banco
                const response = await fetch(`/api/vagas/mapa?patioId=3`);
                if (!response.ok) throw new Error("Falha ao buscar dados do mapa.");
                const mapaData: MapaData = await response.json();
                
                console.log('📊 Dados do mapa Limão recebidos:', {
                    ...
                    sampleBox: mapaData.boxes?.[0],
                    allBoxes: mapaData.boxes
                });
                
                const mappedBoxes: MappedBox[] = mapaData.boxes.map(apiBox => {
                    const position = getBoxPositionFromName(apiBox.nome);
                    // Usar tamanhos otimizados para os boxes BLimao no telhado expandido
                    const isLimaoBox = apiBox.nome.startsWith('BLimao') || apiBox.nome.startsWith('Li');
                ...
                console.log('🎯 Boxes mapeados para renderização:', {
                    totalMapped: mappedBoxes.length,
                    ocupados: mappedBoxes.filter(b => b.status === 'Ocupado').length,
                    sampleMapped: mappedBoxes[0],
                    viewOptions
                });
            } catch (err: any) {
                console.error("Falha ao carregar boxes do pátio:", err);
                throw err;
            }
        };
    ...
    const handleMouseEnterBox = (e: React.MouseEvent, box: MappedBox) => { 
        console.log('Mouse enter box:', { status: box.status, nome: box.nome, veiculo: box.veiculo });
        if (box.status === 'Ocupado') { 
            console.log('Setting tooltip for occupied box');
            setTooltip({ 
                x: e.clientX, 
                y: e.clientY, 
                content: ( 
