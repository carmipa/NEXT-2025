            ...
            <div className="space-y-8">
                {vagasFiltradas.length === 0 ? (
                    ...
                    vagasFiltradas.map((grupo) => {
                        // Calcular dimensões da grade (máximo 20 colunas)
                        const totalVagas = grupo.vagas.length;
                        const colunas = Math.min(20, Math.ceil(Math.sqrt(totalVagas)));
                        const linhas = Math.ceil(totalVagas / colunas);
                                    ...
                                    {grupo.vagas.map((vaga, index) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`
                                                    relative w-full h-full rounded border-2 transition-all duration-200
                ...
                >
                    <div className="flex items-center gap-2 mb-2">
                        <div className={`w-3 h-3 rounded-full ${
                            ...
                            hoverInfo.vaga.status === 'O' ? 'bg-red-500' : 'bg-yellow-500'
                        }`}></div>
