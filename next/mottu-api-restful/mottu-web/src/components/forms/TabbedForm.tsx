...
const TabbedForm: React.FC<TabbedFormProps> = ({ tabs, selectedIndex, onChange }) => {
    return (
        <div className="w-full neumorphic-container">
            <Tab.Group selectedIndex={selectedIndex} onChange={onChange}>
                ...
                <Tab.List className="flex space-x-1 px-1 sm:px-2">
                  ...
                  rounded-t-md transition-colors duration-150 relative group
                  ${tabItem.disabled
                                    ? 'text-slate-600 cursor-not-allowed'
                                    : selected
                                        ? 'text-emerald-700 bg-[var(--neumorphic-bg)]'
                                        : 'text-slate-500 bg-[var(--neumorphic-bg)] hover:text-emerald-700'}
                  ${!tabItem.disabled && 'border-r border-slate-300/60 last:border-r-0'}
                `
                            }
                        >
                            {tabItem.icon}
                    ...
                    <div className="flex-grow border-b border-slate-300"></div>
                </Tab.List>
                ...
                <Tab.Panels className="mt-[-1px]"> {/* Leve sobreposição para conectar */}
                    {tabs.map((tabItem, idx) => (
                        <Tab.Panel
                            key={idx}
                            className="rounded-b-lg rounded-tr-lg p-3 sm:p-5 focus:outline-none border border-t-0 border-slate-200 bg-[var(--neumorphic-bg)]"
                        >
                            {tabItem.content}
                        </Tab.Panel>
                    ))}
